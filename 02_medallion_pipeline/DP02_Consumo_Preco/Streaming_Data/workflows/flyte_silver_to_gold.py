...
# ---------------------------------------------------------------------------
# Task 3: produto analítico principal — janela incremental
# ---------------------------------------------------------------------------
@task(retries=3)
def build_dp_energy_market_api_since(since_date: date) -> int:
    """
    Reconstrói gold.dp_energy_market_api_hourly desde since_date - 1 dia.

    O recuo de 1 dia garante que os lags LAG(24h) e rolling avg 24h do
    primeiro dia novo são calculados corretamente sobre o histórico Silver.
    Idempotente: apaga as linhas da janela e reinsere.
    """
...
# ---------------------------------------------------------------------------
# Task 4: feature table ML — janela incremental
# ---------------------------------------------------------------------------
@task(retries=3)
def build_feat_load_forecasting_api_since(since_date: date, upstream_rows: int) -> int:
    """
    Reconstrói gold.feat_load_forecasting_api_hourly desde since_date - 1 dia.

    Derivada de dp_energy_market_api_hourly com adição de consumo_next_hour = LEAD(1).
    O parâmetro upstream_rows cria dependência explícita sobre o produto analítico.
    Idempotente: apaga as linhas da janela e reinsere.
    """
    _ = upstream_rows
...
def silver_to_gold_api_incremental(since_date: date) -> None:
    """
    Reconstrói Gold apenas para a janela [since_date - 1 dia, hoje].

    Mantém o histórico anterior intacto; garante lags corretos com o recuo de 1 dia.
