...
DP-01 & \path{producao_consumo_streaming_overview.json} & \path{iceberg.gold.dp_energia_balance_hourly} \\
...
\begin{itemize}[leftmargin=*,noitemsep]
  \item \textbf {PASS}: valor dentro dos limites definidos no contrato do DP
  \item \textbf{WARN}: anomalia não crítica que não viola o contrato mas merece atenção, pipeline continua, log emitido, registo persistido em \texttt{quality\_log}
  \item \textbf{FAIL}: violação crítica do contrato, com \texttt{SystemExit} imediato; os dados corrompidos não chegam a dashboards, modelos ML ou APIs
\end{itemize}
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item Check S13 (\texttt{pct\_ok >= 95\%}) transita para WARN (limiar exactamente na fronteira)
  \item Checks S06--S10 (intervalos físicos de meteorologia: temperatura, precipitação, vento, radiação, nebulosidade) transitam para FAIL, bloqueando a promoção para Gold até ao restauro dos dados originais
\end{itemize}

O restauro é imediato via \texttt{--restore}, que elimina as linhas corrompidas da Bronze. A nova execução da Silver confirma que todos os checks regressam a PASS e que a Gold não foi contaminada durante o período de corrupção, uma vez que a Gold filtra explicitamente por \texttt{\_quality\_flag = 'ok'} antes de qualquer join ou cálculo de features.
