...
O MIBEL (\textit{Mercado Ibérico de Electricidade}) é o mercado grossista de eletricidade da Península Ibérica, criado em 2007 como resultado de um acordo bilateral entre Portugal e Espanha com o objetivo de harmonizar os mercados elétricos dos dois países. Em Portugal, a gestão da rede de transporte é da responsabilidade da REN (Rede Energética Nacional), entidade que publica dados horários de produção e consumo, enquanto os preços \textit{day-ahead} são determinados diariamente na bolsa de energia OMIE (Operador do Mercado Ibérico de Energia).
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Prever} a produção elétrica diária e o preço \textit{spot} a partir de variáveis meteorológicas, permitindo ao departamento comercial antecipar decisões de compra e venda de energia com até sete dias de antecedência.
\end{enumerate}

Para responder a este cenário de forma integrada, o grupo construiu a plataforma \textbf{MIBEL Energy Analytics Platform}: um lakehouse local completo, que processa dados históricos desde 2022, implementado com três Data Products independentes, cinco modelos de ML em produção e dashboards operacionais auto-provisionados. A plataforma corre integralmente em Docker Compose, sem dependências de serviços cloud externos, o que garante reprodutibilidade total e independência de custos de infraestrutura.
...
\begin{itemize}[leftmargin=*,noitemsep]
  \item \textbf{Reprodutibilidade}: re-treino com dados históricos da mesma \texttt{feature\_set\_version} produz exatamente o mesmo conjunto de \textit{features} de entrada, independentemente de alterações posteriores à tabela Gold
  \item \textbf{Rastreabilidade}: o registo MLflow associa \texttt{feature\_set\_version} a \texttt{dataset=iceberg.gold.<tabela>}, permitindo auditar que dados alimentaram cada modelo em produção
  \item \textbf{Evolução segura}: uma nova transformação de \textit{feature} implica incrementar o número de versão major, sem quebrar consumidores que ainda referenciam a versão anterior
\end{itemize}
...
\begin{description}[leftmargin=*]
  ...
  \item[\textbf{Gold}] Tabelas Iceberg \textit{managed} com joins \textit{cross-DP}, lags temporais (1h, 24h, 7d), \textit{rolling windows} e flags de negócio calculados em SQL Trino. A Gold é a única fonte de verdade para dashboards, modelos ML e quality gates --- nenhum consumidor acede a Bronze ou Silver diretamente.
\end{description}

\subsection{Decisão de Impacto: Hive para Bronze, Iceberg para Silver e Gold}

A separação deliberada entre Hive (Bronze) e Iceberg (Silver/Gold) é uma das decisões de maior impacto arquitetural do projeto. Em vez de usar um único formato de tabela para todas as camadas, o grupo optou por formatos diferentes com base nas garantias necessárias em cada camada:
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item A combinação das duas tecnologias no mesmo Hive Metastore e no mesmo catálogo Trino permite queries que cruzam Bronze e Silver sem infraestrutura adicional, o que é útil para debugging e validação de transformações
\end{itemize}
...
\begin{enumerate}[leftmargin=*,noitemsep]
  ...
  \item DDL: \texttt{CREATE TABLE IF NOT EXISTS} para Bronze, Silver e Gold (idempotente; re-corridas seguras)
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item \textbf{Flags de controlo granulares}: \texttt{--source \{energycharts|entsoe\}}, \texttt{--start}, \texttt{--end}, \texttt{--today}, \texttt{--full}, \texttt{--clean}, permitindo reprocessamento cirúrgico de intervalos específicos sem re-correr o histórico completo
\end{itemize}
...
\begin{description}[leftmargin=*]
  ...
  \item[\texttt{audit.dataset\_lineage}] Grafo upstream $\rightarrow$ downstream materializado por execução, documentando a cadeia de transformações: \texttt{bronze.consumo\_api\_raw $\rightarrow$ silver.consumo\_api\_hourly $\rightarrow$ gold.dp\_energy\_market\_api\_hourly}. Cada aresta do grafo inclui o \texttt{run\_id}, permitindo reconstruir que runs produziram cada versão dos dados.
\end{description}
...
A estratégia de serving do projeto assenta exclusivamente em \textbf{Grafana 10.4}, que consulta diretamente as tabelas Gold via Trino. Esta abordagem contrasta com uma arquitetura alternativa comum que interpõe uma API REST Python entre o lakehouse e os dashboards. A escolha por Grafana direto sobre Trino foi tomada após comparar as duas opções no contexto do projeto.

Todos os dashboards são auto-provisionados a partir de ficheiros JSON em \texttt{04\_application/grafana/dashboards/} e carregados automaticamente no arranque do Grafana, sem necessidade de configuração manual. O datasource Trino é configurado via variável de ambiente, apontando para o serviço Trino do Docker Compose.
...
\begin{itemize}[leftmargin=*,noitemsep]
  \item \textbf{DP-01}: saldo líquido nacional (MWh = produção $-$ consumo), ratio produção/consumo por hora, contagem de horas de défice e excedente no período selecionado, série temporal horária com banda de tolerância; o dashboard responde à pergunta ``Portugal está em défice energético agora?''
  ...
  \item \textbf{DP-02 Streaming}: dados quase-em-tempo-real via ENTSO-E/Energy-Charts, \textit{rolling average} 24h do consumo, comparação Portugal vs.\ Espanha, indicador de \textit{freshness} com semáforo ligado diretamente à tabela \texttt{audit.pipeline\_runs}
...
A abordagem de qualidade do projeto é diferente do que normalmente se observa em projetos académicos: em vez de validações \textit{best-effort} ou simples logging de anomalias, o grupo implementou quality gates \textbf{bloqueantes} que impedem dados de má qualidade de chegar a consumidores downstream. Esta decisão reflete uma visão de qualidade como propriedade sistémica e não como verificação pontual.
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item \textbf{\textcolor{fail-red}{FAIL}}: violação crítica do contrato --- \texttt{SystemExit} imediato; os dados corrompidos não chegam a dashboards, modelos ML ou APIs
\end{itemize}
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item \textbf{Lag consistency}: \texttt{temp\_lag\_1d} deve corresponder à temperatura observada no dia anterior (tolerância 0,01°C), validando que os lags temporais foram calculados corretamente
...
\addlinespace
\texttt{uniqueness (datahora,} \texttt{process\_date)} & Bronze & 24 grupos & Artefacto de re-ingest idempotente: uma segunda execução sobre o mesmo dia gera duplicados temporários na Bronze. A Silver absorve-os via \texttt{GROUP BY + AVG}, sem propagação para camadas superiores. \\
\addlinespace
\texttt{registos\_por\_dia >= 80} & Bronze & 2 dias & Dias de fronteira do ficheiro histórico com carga parcial ($<96$ registos de 15~min esperados). Não afecta a agregação horária da Silver porque as horas completas são processadas corretamente. \\
\addlinespace
\texttt{horas\_por\_dia >= 23} & Silver & 2 dias & Dias de transição DST (\textit{spring forward}): a hora 2 local não existe, pelo que o dia tem 23 horas. Comportamento esperado do calendário europeu de horário de verão. \\
\bottomrule
\end{tabular}
...
Para evidenciar este mecanismo de forma controlada, foi desenvolvido o script \texttt{corrupt\_bronze.py} (em \texttt{01\_bronze/data\_quality\_demo/}), que injeta padrões de sujidade diretamente nas tabelas Iceberg Bronze via Trino, sem modificar os scripts de ingestão de produção. A motivação é explícita: os pipelines de API (DP-02 Streaming, DP-03) têm validação mínima na ingestão --- nulos são filtrados, mas intervalos físicos e duplicados não são verificados na Bronze. A Silver é, deliberadamente, a única camada de limpeza.

\begin{table}[h!]
\centering
\small
\begin{tabular}{p{2.5cm}p{4cm}p{5.8cm}}
\toprule
\textbf{Tipo} & \textbf{O que injeta} & \textbf{Como Silver responde} \\
\midrule
\texttt{nulls} & \texttt{temperature\_2m = NULL} nas primeiras N linhas & \texttt{\_quality\_flag = 'null\_values'} \\
\addlinespace
\texttt{outofrange} & \texttt{temp=-50°C}, \texttt{radiation=-200\,W/m²}, \texttt{cloud=150\%} & \texttt{\_quality\_flag = 'out\_of\_range'} \\
\addlinespace
\texttt{duplicates} & Cópias de \textit{timestamps} com \texttt{\_ingested\_at=1970} & Silver deduplica via \texttt{ROW\_NUMBER OVER (PARTITION BY ts\_utc ORDER BY \_ingested\_at DESC)} --- mantém o registo mais recente \\
\addlinespace
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item Check S13 (\texttt{pct\_ok >= 95\%}) transita para \textcolor{warn-orange}{WARN} (limiar exatamente na fronteira)
...
O restauro é imediato via \texttt{--restore}, que elimina as linhas corrompidas da Bronze. A re-execução da Silver confirma que todos os checks regressam a \textcolor{pass-green}{PASS} e que a Gold não foi contaminada durante o período de corrupção --- a Gold filtra explicitamente por \texttt{\_quality\_flag = 'ok'} antes de qualquer join ou cálculo de features.

A demo valida empiricamente o papel de cada camada na arquitetura medallion: a Bronze actua como \textit{landing zone} fiel à fonte (sem rejeições nem transformações), e a Silver é a única fronteira de limpeza que impede dados inválidos de contaminar targets de treino ML e visualizações operacionais.
...
\begin{itemize}[leftmargin=*,noitemsep]
  \item Histograma de \textit{duration\_seconds} por execução --- permite detetar degradação de performance ao longo do tempo
  \item Série temporal de \texttt{rows\_gold} por run --- permite detetar gaps de ingestão (runs com zero linhas inseridas na Gold)
  \item Painel de \textit{freshness} com semáforo: \texttt{CURRENT\_DATE - MAX(process\_date)} em dias; verde $<$ 3 dias, amarelo 3--7 dias, vermelho $>$ 7 dias
...
\begin{itemize}[leftmargin=*,noitemsep]
  ...
  \item O custo de um FAIL na pipeline é baixo e conhecido: corrigir a causa raiz, re-correr o pipeline, verificar que os checks passam
...
O DP-02 Streaming tem a sua própria feature table (\texttt{feat\_load\_forecasting\_api\_hourly}) derivada dos dados ENTSO-E/Energy-Charts, com as mesmas colunas que a variante estática mas particionada por \texttt{year/month} para eficiência em ingestões incrementais diárias.
...
\begin{itemize}[leftmargin=*,noitemsep]
  \item \textbf{Tags}: \texttt{dataset} (tabela Gold exata, e.g. \texttt{iceberg.gold.dp\_energia\_balance\_hourly}), \texttt{target}, \texttt{model\_type}, \texttt{domain}
  \item \textbf{Params}: todos os hiperparâmetros do modelo + \texttt{train\_start/end}, \texttt{test\_start/end}, \texttt{n\_train}, \texttt{n\_test}, \texttt{random\_state}
  ...
  \item \textbf{Artefactos}: \texttt{feature\_importances.json} (rastreabilidade do contributo de cada \textit{feature}), \texttt{feature\_columns.json} (lista exata de colunas usadas, para reprodução do treino), \texttt{model/} (pipeline sklearn serializado com scaler + modelo)
...
Duas decisões de ML têm impacto direto na validade das métricas de avaliação e na ausência de \textit{data leakage}:
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{D)} Quatro dashboards Grafana provisionados como código JSON em git, com SQL direto sobre Trino e sem camadas de serving intermédias que possam introduzir \textit{skew}
